...
class BookingControllerTest {
    ...
    private BookingDtoObjects bookingOutDto;

    private BookingDto.BookingDtoBuilder builderIn;
    private BookingDtoObjects.BookingDtoObjectsBuilder builderOut;
    ...
    void setupBuilder() {
        ...
        builderOut = BookingDtoObjects.builder()
                .id(1L)
    ...
    @Test
    void testCreateFailStartTimeIsNull() throws Exception {
        BookingDto bookingInDto = builderIn.start(null).build();
        String json = mapper.writeValueAsString(bookingInDto);
        String error = "В теле Booking отсутствует старт/конец";
        when(mvc.perform(post(URL).header("X-Sharer-User-Id", 1)
                .contentType(MediaType.APPLICATION_JSON).content(json)))
                .thenThrow(new ValidationException("В теле Booking отсутствует старт/конец"));
        mvc.perform(post(URL)
                        .header("X-Sharer-User-Id", 1)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(json))
    ...
    @Test
    void testCreateFailEndTimeIsNull() throws Exception {
        BookingDto bookingInDto = builderIn.end(null).build();
        String json = mapper.writeValueAsString(bookingInDto);
        String error = "В теле Booking отсутствует старт/конец";
        when(mvc.perform(post(URL).header("X-Sharer-User-Id", 1)
                .contentType(MediaType.APPLICATION_JSON).content(json)))
    ...
    void testGetListOfUserBookerEmpty() throws Exception {
        when(bookingService.getListOfUserBooker(1L, State.REJECTED, 0, 10))
    ...
    void testGetListOfUserBookerOneObject() throws Exception {
        bookingOutDto = builderOut.build();
        when(bookingService.getListOfUserBooker(1L, State.WAITING, 0, 1))
    ...
    @Test
    void testGetListOfUserBookerFailState() throws Exception {
        String error = "Unknown state: UNSUPPORTED_STATUS";
        when(bookingService.getListOfUserBooker(1L, State.UNKNOWN, 0, 1))
    ...
    void testGetListBookerOfOwnerItemsEmptyList() throws Exception {
        when(bookingService.getListBookerOfOwnerItems(1L, State.REJECTED, 0, 10))
                .thenReturn(Collections.emptyList());
    ...
    void testGetListBookerOfOwnerItemsOneObject() throws Exception {
        bookingOutDto = builderOut.build();
        when(bookingService.getListBookerOfOwnerItems(1L, State.WAITING, 0, 1))
                .thenReturn(List.of(bookingOutDto));
        mvc.perform(get(URL + "/owner")
                        .header("X-Sharer-User-Id", 1)
                        .param("state", "WAITING")
    ...
    @Test
    void testGetListBookerOfOwnerItemsFailState() throws Exception {
        String error = "Unknown state: UNSUPPORTED_STATUS";
        when(bookingService.getListBookerOfOwnerItems(1L, State.UNKNOWN, 0, 1))
                .thenThrow(new UnsupportedStatusException(error));
        mvc.perform(get(URL + "/owner")
                        .header("X-Sharer-User-Id", 1)
                        .param("state", "qwerty")
